...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, input_ids_ent=None):
        ...
        words_embeddings_ent = self.word_embeddings(input_ids_ent)
        words_embeddings_ent = self.LayerNorm(words_embeddings_ent)
        words_embeddings_ent = self.dropout(words_embeddings_ent)

        return embeddings, words_embeddings_ent
...
class BertAttention(nn.Module):
    def __init__(self, config):
        ...
        self.self_ent = BertSelfAttention(config)
        self.output_ent = BertSelfOutput(config)
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.dense_ent = nn.Linear(config.hidden_size, config.intermediate_size)
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense_ent = nn.Linear(config.intermediate_size, config.hidden_size)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        # self.LayerNorm_ent = BertLayerNorm(100, eps=1e-12)
        self.LayerNorm_ent = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertEntPredictionHead(nn.Module):
    def __init__(self, config, num_norm_labels):
        ...
        self.transform = BertPredictionHeadTransform(config)
        self.decoder = nn.Linear(config.hidden_size, num_norm_labels)
...
class BertPreTrainingHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights, num_norm_labels):
        super(BertPreTrainingHeads, self).__init__()
        self.predictions = BertLMPredictionHead(config, bert_model_embedding_weights)
        self.predictions_ent = BertEntPredictionHead(config, num_norm_labels)
        self.seq_relationship = nn.Linear(config.hidden_size, 2)

    def forward(self, sequence_output, pooled_output):
        prediction_scores = self.predictions(sequence_output)
        seq_relationship_score = self.seq_relationship(pooled_output)
        prediction_scores_ent = self.predictions_ent(sequence_output)
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, output_all_encoded_layers=True):
        ...
        embedding_output, embedding_output_ent = self.embeddings(input_ids, token_type_ids, input_ent)
        encoded_layers = self.encoder(embedding_output,
                                      extended_attention_mask,
                                      embedding_output_ent,
                                      extended_ent_mask,
                                      ent_mask,
...
class BertForPreTraining(PreTrainedBertModel):
    ...
    ```
    """
    def __init__(self, config, num_norm_labels=None):
        super(BertForPreTraining, self).__init__(config)
        self.bert = BertModel(config)
        self.cls = BertPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight, num_norm_labels)
        self.apply(self.init_bert_weights)
        self.num_norm_labels = num_norm_labels

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, 
        input_ent=None, ent_mask=None, next_sentence_label=None, ent_labels=None):
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            ent_ae_loss = loss_fct(prediction_scores_ent.view(-1, self.num_norm_labels), ent_labels.view(-1))
